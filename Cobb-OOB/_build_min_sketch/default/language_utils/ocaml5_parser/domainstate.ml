# 1 "language_utils/ocaml5_parser/utils/domainstate.ml"
...
let idx__major_work_computed = curr in let curr = curr + 1 in
let idx__major_work_todo = curr in let curr = curr + 1 in
let idx__major_gc_clock = curr in let curr = curr + 1 in
...
let idx__pools_to_rescan = curr in let curr = curr + 1 in
let idx__pools_to_rescan_len = curr in let curr = curr + 1 in
let idx__pools_to_rescan_count = curr in let curr = curr + 1 in
...
| Domain_major_work_computed -> idx__major_work_computed
| Domain_major_work_todo -> idx__major_work_todo
| Domain_major_gc_clock -> idx__major_gc_clock
...
| Domain_pools_to_rescan -> idx__pools_to_rescan
| Domain_pools_to_rescan_len -> idx__pools_to_rescan_len
| Domain_pools_to_rescan_count -> idx__pools_to_rescan_count
