...
let[@axiom] list_emp_is_twos (l : int list) = (emp l)#==>(all_twos l)

let[@axiom] list_hd_is_two (l : int list) (l1 : int list) (x : int) (x2 : int)
    =
  (tl l l1 && hd l x && all_twos l1 && x == 2)#==>(all_twos l)

let[@axiom] list_is_twos_destruct (l : int list) (l1 : int list) (x : int)
    ((x2 [@exists]) : int) =
  (tl l l1 && hd l x && all_twos l)#==>(all_twos l1 && x == 2)
