let rec rbtree_gen (inv : int) (color : bool) (h : int) (lo : int) (hi : int) :
    int rbtree =
  if incr lo < hi then
    ...
    if sizecheck h then
      ...
      else Rbtnode (true, Rbtleaf, x, Rbtleaf)
    else if color then
      ...
      if c then
        ...
        let (lt4 : int rbtree) =
          rbtree_gen (subs (subs inv)) false (subs h) lo x
        in
        let (rt4 : int rbtree) =
          rbtree_gen (subs (subs inv)) false (subs h) x hi
        in
