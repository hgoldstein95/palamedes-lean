...
let[@axiom] list_emp_is_even (l : int list) = (emp l)#==>(all_evens l)

let[@axiom] list_hd_is_even (l : int list) (l1 : int list) (x : int) (x2 : int)
    =
  (tl l l1 && hd l x && all_evens l1 && x2 * 2 == x)#==>(all_evens l)

let[@axiom] list_is_even_destruct (l : int list) (l1 : int list) (x : int)
    ((x2 [@exists]) : int) =
