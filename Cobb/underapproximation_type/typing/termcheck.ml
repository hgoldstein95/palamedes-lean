...
let _rec_arg : (string * t prop) list ref = ref []
let init_rec_arg x = _rec_arg := x

let apply_rec_arg1 arg =
  match
...
let apply_rec_arg2 arg1 param1 arg2 =
  match
    List.find_opt (fun (name, _) -> String.equal name "rec_arg2") !_rec_arg
  with
  | Some (_, p) ->
