# 1 "language_utils/ocaml5_parser/utils/misc.ml"
(**************************************************************************)
(*                                                                        *)
(*                                 OCaml                                  *)
(*                                                                        *)
...
let copy_file_chunk ic oc len =
  ...
  let rec copy n =
    if n <= 0 then () else begin
      let r = input ic buff 0 (Zint.min n 0x1000) in
...
module LongString = struct
  ...
  let input_bytes_into tbl ic len =
    ...
    Array.iter (fun str ->
      let chunk = Zint.min !count (Bytes.length str) in
    ...
    for i = 1 to la do
      for j = Zint.max 1 (i - cutoff - 1) to Zint.min lb (i + cutoff + 1) do
        ...
        let best =
          ...
          Zint.min (1 + Zint.min m.(i-1).(j) m.(i).(j-1)) (m.(i-1).(j-1) + cost)
        in
        let best =
          ...
          then best
          else Zint.min best (m.(i-2).(j-2) + cost)
        in
        m.(i).(j) <- best
      done;
    done;
...
module Magic_number = struct
  ...
  let parse s : (info, parse_error) result =
    if String.length s = magic_length then begin
      ...
      let sub_length = Zint.min kind_length (String.length s) in
      let starts_as kind =
