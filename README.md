...
The expected runtime of this script is 30 seconds. The expected output is approximately:
```
...
OOB version
sortedlist,success, 0.71,success, 6.91
...
The data for Figure 19 is generated via two scripts.
...
Cobb's results are generated by running two scripts from the artifact home directory.
```
./run_cobb.sh
```
produces the data for the AE (=Additional Engineering) set of results.

```
./run_cobb_oob.sh
```
produces the data for the OOB (=Out of the Box) set of results. This will return a subset of
the lines returned by `run_cobb.sh`.

Expected run time: 1 hour per script.

Run produces a CSV table to standard output that is also saved to the files `Cobb/cobbResults.txt` 
and `Cobb/cobbOOBResults.txt`, resp.
These table provides the Cobb data points for figure 19. The results from `run_cobb.sh` 
(`cobbResults.txt`) should (approximately) match the table in
section E.2. in the appendix. You may notice the following discrepancy:
...
│   ├── between_5_10_min_sketch
│   ├── between_lo_hi_max_sketch
│   ├── between_lo_hi_min_sketch
│   ├── between_lo_hi_or_zero_max_sketch
│   ├── between_lo_hi_or_zero_min_sketch
...
│   ├── equals2or5andTrue_max_sketch
│   ├── equals2or5andTrue_min_sketch
│   ├── equals2sym_max_sketch
│   ├── equals2sym_min_sketch
...
│   ├── gt5_max_sketch
│   ├── gt5_min_sketch
│   ├── incr_one_tree_max_sketch
│   ├── incr_one_tree_min_sketch
